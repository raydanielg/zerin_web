...
==============================================*/
    ...
    function submitByAjax(formSelector, options = {}) {
        $(document).on('submit', formSelector, function (e) {
            ...
            $('.error-text').text('');
            $('input, textarea, select, .select2-selection').removeClass('is-invalid');
            ...
            $.post({
                ...
                success: function (data) {
                    if (data?.errors) {
                        ...
                        $.each(data.errors, function (field, messages) {
                            if (messages.error_code.startsWith('images')) {
                                let allMessages = Array.isArray(messages.message) ? messages.message.join('<br>') : messages.message;

                            }

                            $(`.error-text[data-error="${messages.error_code}"]`).text(messages.message);
                            let input = $(`[name="${messages.error_code}"]`);
                            if (!input.length && messages.error_code.includes('.')) {
                                let base = messages.error_code.split('.')[0];
                                input = $(`[name="${base}[]"]`).eq(messages.error_code.split('.')[1]);
                            }
                            if (input.length) {
                            ...
                            if (!firstErrorElement) {
                                ...
                                if (input.length) {
                                    ...
                                    firstErrorElement = $(`.error-text[data-error="${messages.error_code}"]`);
                                }
                            }
                        });
                ...
                error: function (xhr) {
                    toastr.error('Something went wrong! Please try again after some times.', {
                        CloseButton: true,
                        ProgressBar: true,
                    });
