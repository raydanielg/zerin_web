...
(function ($) {
 ...
 ==============================================*/
  ...
  ==============================================*/
    ...
    //Additional dynamic data > Custom Input field
    $(document).ready(function () {
        /* == Mian field add ==*/
        $(document).on('click', '.add-filed-cmnBtn', function () {

            let wrapper = $(this).closest('.custom-input_field_wrap');
            wrapper.find('.add-field-top').removeClass('d-none');
            wrapper.find('.empty-estate-addfield').addClass('d-none');
            let newItem = wrapper.find('.custom-main-filed-item').first().clone();

            newItem.removeClass('d-none');
            newItem.find('input').val('');
            newItem.find('select').prop('selectedIndex', 0);

            wrapper.find('.d-flex.flex-column.gap-3').append(newItem);
        });
        /* == Remove main field ==*/
        $(document).on('click', '.cutom_mainItem-RemoveBtn', function () {

            let wrapper = $(this).closest('.custom-input_field_wrap');
            $(this).closest('.custom-main-filed-item').remove();

            if(wrapper.find('.custom-main-filed-item:not(.d-none)').length === 0){
                wrapper.find('.empty-estate-addfield').removeClass('d-none');
                wrapper.find('.add-field-top').addClass('d-none');
            }
        });
        /* == Select type change ==*/
        $(document).on('change', '.custom-input-field_select', function(){
            let mainItem = $(this).closest('.custom-main-filed-item');
            let value = $(this).val();

            // hide all inside this item only
            mainItem.find('.flexible-placeholder').hide();
            mainItem.find('.flexible-file-format').hide();
            mainItem.find('.flexible-radio').hide();
            mainItem.find('.checkmark-flexible-wrapper').hide();

            if(value === 'text' || value === 'number' || value === 'date' || value === 'email' || value === 'phone'){
                mainItem.find('.flexible-placeholder').show();
            }
            else if(value === 'file_upload'){
                mainItem.find('.flexible-file-format').show();
            }
            else if(value === 'radio_group'){
                mainItem.find('.flexible-radio').show();
            }
            else if(value === 'check_boxes'){
                mainItem.find('.checkmark-flexible-wrapper').show();
            }

        });
        /* == Checkmark add remove ==*/
        $(document).on('click', '.add-checkmark-btn', function () {

            let wrapper = $(this).closest('.checkmark-flexible-wrapper');
            let newItem = wrapper.find('.add-checkmark-item').first().clone();

            newItem.removeClass('d-none');
            newItem.find('input').val('');

            wrapper.find('.d-flex.flex-column').append(newItem);
        });
        $(document).on('click', '.checkmark-item-removeBtn', function () {
            $(this).closest('.add-checkmark-item').remove();
        });
        /* == Radio add remove ==*/
        $(document).on('click', '.add-radio-btn', function () {

            let wrapper = $(this).closest('.flexible-radio');
            let newItem = wrapper.find('.add-radio-item').first().clone();
        ...
        $(document).on('click', '.radio-item-removeBtn', function () {
            $(this).closest('.add-radio-item').remove();
        });
    }); 
})(jQuery);
